...
object CheckCaptures:
  ...
  /** An attachment requesting `unsafeDiscardUses` semantics on an Apply: uses
   *  inside the call are not recorded into enclosing environments. Lets a
   *  compiler-internal rewriter (the REPL eval-binding rewriter) suppress
   *  capture propagation for synthesised value references without emitting a
   *  source-level call to `caps.unsafe.unsafeDiscardUses`, which would be
   *  rejected in safe mode. Sticky so the marker survives the tree copies
   *  between the PostTyper-stage rewriter and the capture-check phase. */
...
// ---- Rechecking operations for different kinds of trees----------------------
    ...
    override def recheckApply(tree: Apply, pt: Type)(using Context): Type =
      ...
      if meth == defn.Caps_unsafeAssumePure then
        ...
        // A compiler-internal rewriter asked for `unsafeDiscardUses` semantics
        // on this whole call without going through the (safe-mode-rejected)
        // `caps.unsafe.unsafeDiscardUses` symbol. Recheck the application
        // normally, but with use recording suppressed for everything inside it.
