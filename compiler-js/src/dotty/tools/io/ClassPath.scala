...
object ClassPath {
  ...
  def expandManifestPath(jarPath: String): List[URL] = {
    ...
    new Jar(file).classPathElements map (elem =>
      specToURL(elem, baseDir) getOrElse new URL((baseDir / elem).toURI.toString)
    )
  }
  ...
  def specToURL(spec: String, basedir: Directory): Option[URL] =
    try
      ...
      if uri.isAbsolute() then Some(new URL(uri.toString))
      else
        Some(new URL(basedir.resolve(Path(spec)).toURI.toString))
    catch
