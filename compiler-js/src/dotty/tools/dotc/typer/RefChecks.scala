...
object RefChecks {
  ...
  private def addMissingMethodsActionPatch(
    ...
    // NB (scala3-js override): the JVM build's `interfaces.SourceFile` is a Java
    // interface, so `content()` auto-applies and `.content` also works. Our JS
    // `interfaces.SourceFile` is a Scala trait declaring `def content` (no parens),
    // so this must drop the parens. This is the ONLY reason this file is overridden.
