...
object Pickler {
  ...
  /** Synchronous stub of AsyncTastyHolder for Scala.js: scala.concurrent
   *  primitives don't link, and we don't pipeline anyway. Methods compute
   *  state inline.
   */
  class AsyncTastyHolder private (
    ...
    private var writtenState: Option[AsyncTastyHolder.State] = None
    private var apiComplete: Boolean = incCallback == null
    ...
    def sync(): Option[BufferingReporter] =
      val optState =
        if _cancelled || !apiComplete then None
        else writtenState
