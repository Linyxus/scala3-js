...
trait WrappedProperties extends PropertiesTrait {
  ...
  // PropertiesTrait in the Scala.js stdlib declares these as non-null `String`, but the underlying
  // implementations may return null. Cast through to keep the signature compatible.
  override def setProp(name: String, value: String): String = wrap(super.setProp(name, value)).orNull.asInstanceOf[String]
  override def clearProp(name: String): String              = wrap(super.clearProp(name)).orNull.asInstanceOf[String]
