...
object Build {
  ...
  val buildBinary = taskKey[File]("Build a standalone `scala3` executable with bun (embeds main.js + archives)")
  ...
  lazy val `scala3-compiler-sjs` = project.in(file("compiler-js"))
    ...
    .settings(
      ...
      // Task to build a standalone `scala3` executable via `bun build --compile`.
      // Embeds the fully-optimized main.js plus the packed classpath/linker
      // archives as file assets; a generated cli.ts runs main.js as a classic
      // script (see compiler-js/README.md).
      buildBinary := {
        val s = streams.value
        ...
        val cliContent =
          s"""// Generated by sbt (project/Build.scala, buildBinary). Do not edit.
        ...
        val bunArgs = Seq("bun", "build", "--compile", "--minify",
          cliTs.getAbsolutePath, "--outfile", outFile.getAbsolutePath)
        s.log.info(s"Building standalone binary: ${bunArgs.mkString(" ")}")
        val exit = scala.sys.process.Process(bunArgs, baseDir).!
        if (exit != 0) sys.error(s"bun build --compile failed with exit code $exit")

        s.log.info(s"Built standalone binary at $outFile (${outFile.length() / (1024 * 1024)} MB)")
        outFile
      },
    )
