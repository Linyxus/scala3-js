...
object Build {
   ...
   *  archives + suite-specific vars passed via env. Shared by the JSON-REPL eval
   *  tests and the CLI-REPL transcript tests. */
  private def runScriptedTests(
      classpathBin: File, linkerLibsBin: File, mainJs: File,
      extraEnv: Seq[(String, String)], args: Seq[String],
      cwd: File, failOnError: Boolean, log: sbt.util.Logger): Unit = {
    val env = Seq(
  ...
  /** Release matrix for `buildBinaryAll`: bun `--target` -> output file name.
   *  main.js and the packed archives are platform-independent, so only the
   *  embedded bun runtime differs per target. */
  val binaryTargets: Seq[(String, String)] = Seq(
    "bun-darwin-arm64" -> "scala3-darwin-arm64",
    "bun-darwin-x64"   -> "scala3-darwin-x64",
    "bun-linux-x64"    -> "scala3-linux-x64",
    "bun-linux-arm64"  -> "scala3-linux-arm64",
    "bun-windows-x64"  -> "scala3-windows-x64.exe",
  )

  /** Invoke `bun build --compile` for one target (None = host platform). */
  private def runBunCompile(cliTs: File, target: Option[String], outFile: File, log: Logger): Unit = {
    val targetArgs = target.map(t => Seq(s"--target=$t")).getOrElse(Nil)
  ...
  // Generate a Scala source embedding compiler.properties values for the
  // Scala.js compiler build, where classpath resource loading
  // (getResourceAsStream) is unavailable. Mirrors generateCompilerProperties.
  lazy val generateCompilerPropertiesSource: Def.Initialize[Task[Seq[File]]] = Def.task {
    ...
    val entries = Seq(
      ...
      "maven.version.number" -> version.value,
      "git.hash" -> VersionUtil.gitHash,
      "copyright.string" -> s"Copyright 2002-$currentYear, LAMP/EPFL"
    )
    val mapEntries = entries
      .map { case (k, v) => s"    ${scalaStringLiteral(k)} -> ${scalaStringLiteral(v)}" }
      .mkString(",\n")
    val data =
      s"""package dotty.tools.dotc.config
         |
         |// Generated by sbt (project/Build.scala, generateCompilerPropertiesSource).
         |// Do not edit. Embeds compiler.properties for the Scala.js build, where
         |// classpath resource loading (getResourceAsStream) is unavailable.
         |private[config] object CompilerPropertiesData {
         |  val entries: Map[String, String] = Map(
         |$mapEntries
         |  )
         |}
         |""".stripMargin
    val file = (Compile / sourceManaged).value /
      "dotty" / "tools" / "dotc" / "config" / "CompilerPropertiesData.scala"
    if (!(file.exists && IO.read(file) == data)) {
      IO.write(file, data)
    }
    Seq(file)
  ...
  lazy val allProjects: Seq[Project] = Seq(
    ...
    `scala3-compiler-sjs`,
    `scala3-compiler-cli-sjs`,
    `scala3-compiler-browser-sjs`,
    `scala3-repl-sjs`,
    `scala3-repl-cli-sjs`,
    `scala3-repl-json-sjs`,
    sjsSandbox,
    sjsJUnitTests,
    sjsCompilerTests,
    `community-build`,
  ...
  lazy val `scala3-compiler-sjs` = project.in(file("compiler-js"))
    ...
    .settings(
      ...
      Compile / resourceGenerators += generateCompilerProperties.taskValue,
      // Embed compiler.properties as a Scala source, since Scala.js cannot load
      // it from the classpath at runtime (getResourceAsStream is unavailable).
      Compile / sourceGenerators += generateCompilerPropertiesSource.taskValue,
      ...
      // fully-optimized main.js, the packed classpath/linker archives, and a
      // generated cli.ts entrypoint. The cli.ts embeds the archives + main.js
      // as file assets and runs main.js (Scala.js NoModule, which expects
      // `this === globalThis`) as a classic script. Returns the cli.ts file.
      prepareBinaryAssets := {
        val s = streams.value
        // Packed archives (live in the project target dir, next to the link output dirs)
        val classpathBin = packClasspath.value
        val linkerLibsBin = packLinkerLibs.value
        val baseDir = classpathBin.getParentFile

        // Fully-optimized (minified) compiler
        val _ = (Compile / fullLinkJS).value
        val optDir = (Compile / fullLinkJS / scalaJSLinkerOutputDirectory).value
        val mainJs = optDir / "main.js"
        if (!mainJs.exists()) sys.error(s"fullLinkJS output not found: $mainJs")

        // Generate the bun entrypoint with asset paths relative to baseDir
        val relMain = baseDir.toPath.relativize(mainJs.toPath).toString.replace('\\', '/')
        val cliTs = baseDir / "cli.ts"
        val cliContent =
          s"""// Generated by sbt (project/Build.scala, prepareBinaryAssets). Do not edit.
             |// Standalone `scala3` entrypoint: embeds the compiler and its packed
             |// classpath/linker archives, then runs main.js (Scala.js NoModule,
             |// which expects `this === globalThis`) as a classic script.
             |import classpathBin from "./${classpathBin.getName}" with { type: "file" };
             |import linkerLibsBin from "./${linkerLibsBin.getName}" with { type: "file" };
             |import mainJsPath from "./$relMain" with { type: "file" };
             |import { readFileSync } from "node:fs";
             |import { createRequire } from "node:module";
             |
             |(globalThis as any).require = createRequire(import.meta.url);
             |(globalThis as any).__DOTTY_CLASSPATH_BIN__ = classpathBin;
             |(globalThis as any).__DOTTY_LINKER_LIBS_BIN__ = linkerLibsBin;
             |
             |const mainSrc = readFileSync(mainJsPath, "utf-8");
             |(0, eval)(mainSrc);
             |""".stripMargin
        IO.write(cliTs, cliContent)
        s.log.info(s"Prepared bun entrypoint at $cliTs")
        cliTs
      },
      // Build a standalone `scala3` executable for the host platform.
      buildBinary := {
        val cliTs = prepareBinaryAssets.value
        val distDir = (LocalRootProject / baseDirectory).value / "dist"
        IO.createDirectory(distDir)
        val outFile = distDir / "scala3"
        runBunCompile(cliTs, None, outFile, streams.value.log)
        outFile
      },
      // Build standalone `scala3` executables for the full release matrix.
      buildBinaryAll := {
        val cliTs = prepareBinaryAssets.value
        val log = streams.value.log
        val distDir = (LocalRootProject / baseDirectory).value / "dist"
        IO.createDirectory(distDir)
        val outputs = binaryTargets.map { case (target, name) =>
          val outFile = distDir / name
          runBunCompile(cliTs, Some(target), outFile, log)
          outFile
        }
        log.info(s"Built ${outputs.size} binaries in $distDir:\n  ${outputs.map(_.getName).mkString("\n  ")}")
        outputs
      },
    )

  /** Node CLI wrapper for the Scala.js compiler library. */
  lazy val `scala3-compiler-cli-sjs` = project.in(file("compiler-js-cli"))
    .dependsOn(`scala3-compiler-sjs`)
    .enablePlugins(DottyJSPlugin)
    .settings(
      name          := "scala3-compiler-cli-sjs",
      moduleName    := "scala3-compiler-cli",
      version       := dottyVersion,
      scalaVersion  := dottyNonBootstrappedVersion,
      crossPaths    := true,
      autoScalaLibrary := false,
      ...
      libraryDependencies +=
        ("org.scala-js" %% "scalajs-library" % scalaJSVersion % Provided).cross(CrossVersion.for3Use2_13),
      target := target.value / "scala3-compiler-cli-sjs",
      publish / skip := true,
      bspEnabled := false,
      ...
      bundleLibs := {
        val sourceLib = (`scala3-compiler-sjs` / bundleLibs).value
        val outputDir = (Compile / fastLinkJS / scalaJSLinkerOutputDirectory).value
        val libDir = outputDir.getParentFile / "lib"
        if (libDir.exists()) IO.delete(libDir)
        IO.createDirectory(libDir.getParentFile)
        IO.copyDirectory(sourceLib, libDir)
        streams.value.log.info(s"Bundled libraries at $libDir")
        libDir
      },
      packClasspath := {
        val source = (`scala3-compiler-sjs` / packClasspath).value
        val outputDir = (Compile / fastLinkJS / scalaJSLinkerOutputDirectory).value
        val outputFile = outputDir.getParentFile / "classpath.bin"
        IO.createDirectory(outputFile.getParentFile)
        IO.copyFile(source, outputFile)
        outputFile
      },
      packLinkerLibs := {
        val source = (`scala3-compiler-sjs` / packLinkerLibs).value
        val outputDir = (Compile / fastLinkJS / scalaJSLinkerOutputDirectory).value
  ...
  /** Browser API wrapper exporting `DottyCompiler`. */
  lazy val `scala3-compiler-browser-sjs` = project.in(file("compiler-js-browser"))
    .dependsOn(`scala3-compiler-sjs`)
    .enablePlugins(DottyJSPlugin)
    .settings(
      name          := "scala3-compiler-browser-sjs",
      moduleName    := "scala3-compiler-browser",
      version       := dottyVersion,
      scalaVersion  := dottyNonBootstrappedVersion,
      crossPaths    := true,
      autoScalaLibrary := false,
      ...
      libraryDependencies +=
        ("org.scala-js" %% "scalajs-library" % scalaJSVersion % Provided).cross(CrossVersion.for3Use2_13),
      target := target.value / "scala3-compiler-browser-sjs",
      publish / skip := true,
      bspEnabled := false,
      scalaJSLinkerConfig ~= { _.withESFeatures(_.withESVersion(ESVersion.ES2018)) },
    )
  ...
  lazy val `scala3-repl-json-sjs` = project.in(file("repl-js-json"))
    ...
    .enablePlugins(DottyJSPlugin)
    .settings(
      name          := "scala3-repl-json-sjs",
      moduleName    := "scala3-repl-json",
      version       := dottyVersion,
      scalaVersion  := dottyNonBootstrappedVersion,
      crossPaths    := true,
      autoScalaLibrary := false,
      ...
      libraryDependencies +=
        ("org.scala-js" %% "scalajs-library" % scalaJSVersion % Provided).cross(CrossVersion.for3Use2_13),
      target := target.value / "scala3-repl-json-sjs",
      publish / skip := true,
      bspEnabled := false,
      scalaJSUseMainModuleInitializer := true,
      ...
      Test / test := runScriptedTests(
        ...
        Nil, baseDirectory.value, failOnError = true, streams.value.log),
      updateEvalChecks := runScriptedTests(
        (`scala3-compiler-cli-sjs` / packClasspath).value,
        (`scala3-compiler-cli-sjs` / packLinkerLibs).value,
        { val _ = (Test / fastLinkJS).value
          (Test / fastLinkJS / scalaJSLinkerOutputDirectory).value / "main.js" },
